...
|---|---|---|
| FND-01 | Fresh-world, reload, dedicated-server, and remote-client test procedures | `test` currently reports no source coverage. Rank-3 reference behavior for fresh-world, reload, dedicated-server, and remote-client test procedures captured in [fnd-01-evidence.md](fnd-01-evidence.md); rank-1 confirmation pending. |
...
| AUT-04 | Standalone artifice and utility device behavior (for example Arcane Ear, Condenser, and Levitator) | Device blocks and tiles exist under `common/blocks/devices` and `common/tiles/devices`; behavior is unverified. Rank-3 reference behavior for the standalone artifice devices captured in [aut-04-evidence.md](aut-04-evidence.md); rank-1 confirmation pending. |
| WLD-01 | Aura generation, Vis and Flux persistence, and dimension lifecycle | Aura, Vis, and Flux persistence have not passed fresh-world or reload validation. Rank-3 reference behavior for the aura substrate captured in [wld-01-evidence.md](wld-01-evidence.md); rank-1 confirmation pending. |
| WLD-02 / WLD-03 | Magical tree growth, aura/Flux changes, rift lifecycle, taint growth, and cleansing | Sapling configured features return `null`; rift/stabilization behavior needs verification. Rank-3 reference behavior for renewable Greatwood/Silverwood growth and worldgen captured in [wld-02-evidence.md](wld-02-evidence.md); rank-1 confirmation pending. Rank-3 reference behavior for flux rifts, taint, stabilization, and cleansing captured in [wld-03-evidence.md](wld-03-evidence.md); rank-1 confirmation pending. |
| WLD-04 | Eldritch progression, structures, bosses, and endgame unlocks | The endgame may be unreachable; no complete survival-path evidence exists yet. Rank-3 reference behavior for Eldritch progression, structures, bosses, and endgame unlocks captured in [wld-04-evidence.md](wld-04-evidence.md); rank-1 confirmation pending. |
| WLD-05 | Creature spawning, AI, combat, drops, constructs, and non-focus projectiles | Monster, construct, and projectile entity classes exist under `common/entities`; BETA26 behavior is unvalidated. Rank-3 reference behavior for creature spawning, AI, combat, drops, constructs, and projectiles captured in [wld-05-evidence.md](wld-05-evidence.md); rank-1 confirmation pending. |
| PLY-01 | Player equipment, Curios state, effects, Warp, and Warp Ward | Warp capability, events, and Warp Ward potion exist (`PlayerWarp`, `WarpEvents`, `PotionWarpWard`); behavior is unverified. Rank-3 reference behavior for player equipment, Curios state, effects, Warp, and Warp Ward captured in [ply-01-evidence.md](ply-01-evidence.md); rank-1 confirmation pending. |
| CLI-01 | Research, crafting, and device screens with correct server synchronization — the screen inventory, the container-to-screen wiring, and the interaction mechanics | Workboard acceptance "All core menus open and work without client-only state."; defect "Core research, crafting, and device screens need synchronization and interaction verification." ("Menus may not open or may show stale state."; `CLI-01` row in [deferred-issues.md](deferred-issues.md)). Rank-3 reference behavior for menus, screens, and GUI interaction mechanics captured in [cli-01-evidence.md](cli-01-evidence.md); rank-1 confirmation pending. |
| CLI-02 | Caster, beam, particle, and focus visuals — the client FX pipeline, its packet family, and the beam, particle, and focus-effect renders | Workboard acceptance "Effects are visible to nearby clients and do not crash dedicated servers."; defect "Caster, beam, particle, and focus visuals need dedicated-server-safe restoration." ("Magic lacks expected feedback or can cause client/server crashes."; `CLI-02` row in [deferred-issues.md](deferred-issues.md)). Rank-3 reference behavior for caster, beam, particle, and focus visuals captured in [cli-02-evidence.md](cli-02-evidence.md); rank-1 confirmation pending. |
| CLI-03 | Models, textures, sounds, and entity renders — the asset sweep: the resource payload and its port backlog, the JSON and OBJ model layer, the texture directories, the sound registry, and the forty-three entity and twenty-three tile renderer registrations | Workboard acceptance "No core BETA26 content uses placeholder or missing assets."; defect "Placeholder or missing models, textures, sounds, and renders require an asset sweep." ("Presentation is incomplete even if systems function."; `CLI-03` row in [deferred-issues.md](deferred-issues.md)). Rank-3 reference behavior for models, textures, sounds, and entity renders captured in [cli-03-evidence.md](cli-03-evidence.md); rank-1 confirmation pending. |
