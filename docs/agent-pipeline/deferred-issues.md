...
| FND-04 | Shared aspect attribution, aspect containers, and lookup are not established for scanning or Essentia processing. | Scans and Essentia handling can disagree on item aspects, so every dependent repair rests on unverified data. | Container base-tag masking reproduced and repaired in the first slice; independent specification and quality review passed. See `fnd-04-container-lookup-evidence.md`. Public lookup routing is repaired and independently approved; REF-0011 confirms only the original Aer crystal AIR1 display. Missing seven-type culling is repaired and independently approved. The bound iron/diamond startup loss is repaired; ordinary tag lifecycle and real reload passed independent review (28 JUnit and 21 GameTests). REF-0014 confirms their original displayed values. Recipe generation is repaired and independently approved: the fifth slice implements inspected four-category derivation with actual reload coverage, and its repair batch closed the culling tie-order and direct-boundary precedence defects with RED-verified regressions (committed `c6c7b1b`). Entity attribution is repaired and independently approved (sixth slice `a96faf6`): one ordered store, the BETA26 last-match-wins lookup with typed NBT filters and the player rule, owned copies, and the creeper/pech/wisp/taint-seed/frame/painting restorations. Container capacities are repaired and independently approved (seventh slice `59f53fa`): a 21-row census with four RED-first repairs (void-jar acceptance/accounting, alembic acceptance, phial-into-jar atomic transfer) and 63/63 GameTests. Other attribution (seed/bonus fidelity, complex item registration) and end-to-end consumer evidence remain open. | Reviewed `ae9a183` FND-01 automated start gate satisfied; `ACTIVE`, seven reviewed slices; latest `59f53fa` (container capacities); remote-client and consumer acceptance pending. |
